...
\section{Module này làm gì?}\label{module-nuxe0y-luxe0m-guxec}
...
\section{Kiến trúc tổng quan (dễ
...
\section{Dashboard (Trang chủ tổng
quan)}\label{dashboard-trang-chux1ee7-tux1ed5ng-quan}

\subsection{User thấy gì trên
...
\subsection{Biểu đồ 7 ngày hoạt động thế
...
\subsection{File code liên quan}\label{file-code-liuxean-quan}
...
\subsection{Cơ chế cấp huy hiệu (từng
...
\subsection{Cấu trúc database}\label{cux1ea5u-truxfac-database}
...
\subsection{File code liên quan}\label{file-code-liuxean-quan-1}
...
\section{Notification (Thông báo \& nhắc
nhở)}\label{notification-thuxf4ng-buxe1o-nhux1eafc-nhux1edf}

\subsection{Ba loại thông báo}\label{ba-loux1ea1i-thuxf4ng-buxe1o}
...
\subsection{Luồng bật nhắc tập (ví
...
\subsection{Lưu ý quan trọng về Expo
...
\subsection{Khôi phục nhắc khi mở lại
...
\subsection{File code liên quan}\label{file-code-liuxean-quan-2}
...
\section{Luồng hoạt động tổng
hợp}\label{luux1ed3ng-houx1ea1t-ux111ux1ed9ng-tux1ed5ng-hux1ee3p}

\subsection{Kịch bản A: User mở app buổi
...
\subsection{Kịch bản B: User hoàn thành buổi
...
\subsection{Kịch bản C: User bật nhắc uống
...
\section{Backend (Database)}\label{backend-database}

\subsection{Migration cần chạy}\label{migration-cux1ea7n-chux1ea1y}
...
\subsection{Bảng và cột module tạo
...
\section{Kiểm thử}\label{kiux1ec3m-thux1eed}

\subsection{Lệnh chạy test tự
động}\label{lux1ec7nh-chux1ea1y-test-tux1ef1-ux111ux1ed9ng}

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{npm}\NormalTok{ run test:badges    }\CommentTok{\# test logic huy hieu}
\ExtensionTok{npm}\NormalTok{ run test:weekly    }\CommentTok{\# test normalizeWeeklyWorkouts}
\ExtensionTok{npm}\NormalTok{ test               }\CommentTok{\# chay tat ca test}
\ExtensionTok{npm}\NormalTok{ run typecheck      }\CommentTok{\# kiểm tra TypeScript}

\end{Highlighting}
\end{Shaded}

\subsection{Checklist test thủ
...
\section{Tóm tắt nhanh}\label{tuxf3m-tux1eaft-nhanh}
...
\section{Giải thích code \& hàm chi
...
\subsection{\texorpdfstring{\texttt{getDashboardSummary(userId)} ---
...
\subsection{\texorpdfstring{\texttt{normalizeWeeklyWorkouts(raw)} ---
...
\ImportTok{export} \KeywordTok{function} \FunctionTok{normalizeWeeklyWorkouts}\NormalTok{(raw}\OperatorTok{:} \DataTypeTok{unknown}\NormalTok{)}\OperatorTok{:}\NormalTok{ WeeklyWorkoutDay[] \{}
  \KeywordTok{const}\NormalTok{ base }\OperatorTok{=} \FunctionTok{buildEmptyWeekly}\NormalTok{()}\OperatorTok{;}  \CommentTok{// tao san 7 ngay, moi ngay workouts=0, calories=0}
  ...
  \ControlFlowTok{for}\NormalTok{ (}\KeywordTok{const}\NormalTok{ item }\KeywordTok{of}\NormalTok{ raw) \{}
    \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\NormalTok{item }\OperatorTok{||} \KeywordTok{typeof}\NormalTok{ item }\OperatorTok{!==} \StringTok{\textquotesingle{}object\textquotesingle{}}\NormalTok{) }\ControlFlowTok{continue}\OperatorTok{;}
    \KeywordTok{const}\NormalTok{ row }\OperatorTok{=}\NormalTok{ item }\ImportTok{as} \BuiltInTok{Record}\OperatorTok{\textless{}}\DataTypeTok{string}\OperatorTok{,} \DataTypeTok{unknown}\OperatorTok{\textgreater{};}
    \KeywordTok{const}\NormalTok{ date }\OperatorTok{=} \FunctionTok{normalizeDateString}\NormalTok{(}\BuiltInTok{String}\NormalTok{(row}\OperatorTok{.}\AttributeTok{date} \OperatorTok{??} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{))}\OperatorTok{;} \CommentTok{// "2026{-}06{-}30 00:00:00" {-}\textgreater{} "2026{-}06{-}30"}
...
\NormalTok{    map}\OperatorTok{.}\FunctionTok{set}\NormalTok{(date}\OperatorTok{,}\NormalTok{ \{}
\NormalTok{      date}\OperatorTok{,}
\NormalTok{      workouts}\OperatorTok{:} \BuiltInTok{Number}\NormalTok{(row}\OperatorTok{.}\AttributeTok{workouts} \OperatorTok{??} \DecValTok{0}\NormalTok{)}\OperatorTok{,}
\NormalTok{      calories\_burned}\OperatorTok{:} \BuiltInTok{Number}\NormalTok{(row}\OperatorTok{.}\AttributeTok{calories\_burned} \OperatorTok{??} \DecValTok{0}\NormalTok{)}\OperatorTok{,}
\NormalTok{    \})}\OperatorTok{;}
\NormalTok{  \}}
...
\subsection{\texorpdfstring{\texttt{buildDashboardSummaryClient(userId)}
--- fallback
...
\subsection{\texorpdfstring{\texttt{loadDashboard()} ---
    ...
    \ControlFlowTok{if}\NormalTok{ (prefs}\OperatorTok{?.}\AttributeTok{badge\_notifications\_enabled}\NormalTok{) \{}
      ...
      \ControlFlowTok{for}\NormalTok{ (}\KeywordTok{const}\NormalTok{ badge }\KeywordTok{of}\NormalTok{ badgeList) \{}
        \ControlFlowTok{if}\NormalTok{ (badge}\OperatorTok{.}\AttributeTok{earned} \OperatorTok{\&\&} \OperatorTok{!}\NormalTok{previousEarned}\OperatorTok{.}\FunctionTok{has}\NormalTok{(badge}\OperatorTok{.}\AttributeTok{id}\NormalTok{)) \{}
          \KeywordTok{void} \FunctionTok{notifyBadgeEarned}\NormalTok{(}\StringTok{\textquotesingle{}Huy hieu moi\textquotesingle{}}\OperatorTok{,} \VerbatimStringTok{\textasciigrave{}Ban vua mo khoa "}\SpecialCharTok{$\{}\NormalTok{badge}\OperatorTok{.}\AttributeTok{title}\SpecialCharTok{\}}\VerbatimStringTok{"\textasciigrave{}}\NormalTok{)}\OperatorTok{;}
\NormalTok{        \}}
\NormalTok{      \}}
\NormalTok{    \}}
...
\subsection{\texorpdfstring{\texttt{WeeklyProgressChart} --- biểu đồ 7
...
\subsection{\texorpdfstring{\texttt{isBadgeEarned(badge,\ stats)} ---
...
\subsection{\texorpdfstring{\texttt{loadBadgeStats(userId)} ---
...
\subsection{\texorpdfstring{\texttt{syncUserBadges(userId)} --- cấp huy
hiệu tự
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Rut gon tu badgeService.ts; code that co kiem tra badgesError.}
\ImportTok{export} \KeywordTok{async} \KeywordTok{function} \FunctionTok{syncUserBadges}\NormalTok{(userId}\OperatorTok{:} \DataTypeTok{string}\NormalTok{)}\OperatorTok{:} \BuiltInTok{Promise}\OperatorTok{\textless{}}\NormalTok{BadgeWithStatus[]}\OperatorTok{\textgreater{}}\NormalTok{ \{}
  \KeywordTok{const}\NormalTok{ [\{ data}\OperatorTok{:}\NormalTok{ badges}\OperatorTok{,}\NormalTok{ error}\OperatorTok{:}\NormalTok{ badgesError \}}\OperatorTok{,}\NormalTok{ stats}\OperatorTok{,}\NormalTok{ \{ data}\OperatorTok{:}\NormalTok{ earned \}] }\OperatorTok{=} \ControlFlowTok{await} \BuiltInTok{Promise}\OperatorTok{.}\FunctionTok{all}\NormalTok{([}
\NormalTok{    supabase}\OperatorTok{.}\FunctionTok{from}\NormalTok{(}\StringTok{\textquotesingle{}badges\textquotesingle{}}\NormalTok{)}\OperatorTok{.}\FunctionTok{select}\NormalTok{(}\StringTok{\textquotesingle{}*\textquotesingle{}}\NormalTok{)}\OperatorTok{.}\FunctionTok{order}\NormalTok{(}\StringTok{\textquotesingle{}sort\_order\textquotesingle{}}\OperatorTok{,}\NormalTok{ \{ ascending}\OperatorTok{:} \KeywordTok{true}\NormalTok{ \})}\OperatorTok{,}
...
\NormalTok{  ])}\OperatorTok{;}
  ...
  \ControlFlowTok{if}\NormalTok{ (badgesError }\OperatorTok{||} \OperatorTok{!}\NormalTok{badges}\OperatorTok{?.}\AttributeTok{length}\NormalTok{) }\ControlFlowTok{return} \FunctionTok{getBadgesWithStatus}\NormalTok{(userId)}\OperatorTok{;}
  ...
  \ControlFlowTok{if}\NormalTok{ (toAward}\OperatorTok{.}\AttributeTok{length} \OperatorTok{\textgreater{}} \DecValTok{0}\NormalTok{) \{}
    \KeywordTok{const}\NormalTok{ \{ error \} }\OperatorTok{=} \ControlFlowTok{await}\NormalTok{ supabase}\OperatorTok{.}\FunctionTok{from}\NormalTok{(}\StringTok{\textquotesingle{}user\_badges\textquotesingle{}}\NormalTok{)}\OperatorTok{.}\FunctionTok{insert}\NormalTok{(}
...
\NormalTok{    )}\OperatorTok{;}
    \ControlFlowTok{if}\NormalTok{ (error }\OperatorTok{\&\&}\NormalTok{ error}\OperatorTok{.}\AttributeTok{code} \OperatorTok{!==} \StringTok{\textquotesingle{}23505\textquotesingle{}}\NormalTok{) }\BuiltInTok{console}\OperatorTok{.}\FunctionTok{warn}\NormalTok{(error}\OperatorTok{.}\AttributeTok{message}\NormalTok{)}\OperatorTok{;}
\NormalTok{  \}}
...
\subsection{\texorpdfstring{\texttt{getNotifications()} --- lazy-load an
toàn Expo
...
\KeywordTok{async} \KeywordTok{function} \FunctionTok{getNotifications}\NormalTok{()}\OperatorTok{:} \BuiltInTok{Promise}\OperatorTok{\textless{}}\NormalTok{NotificationsModule }\OperatorTok{|} \DataTypeTok{null}\OperatorTok{\textgreater{}}\NormalTok{ \{}
  \ControlFlowTok{if}\NormalTok{ (isExpoGo) }\ControlFlowTok{return} \KeywordTok{null}\OperatorTok{;}  \CommentTok{// Expo Go {-}\textgreater{} khong dung notification}

  \ControlFlowTok{if}\NormalTok{ (notificationsModule }\OperatorTok{===} \KeywordTok{undefined}\NormalTok{) \{}
    \ControlFlowTok{try}\NormalTok{ \{}
\NormalTok{      notificationsModule }\OperatorTok{=} \ControlFlowTok{await} \ImportTok{import}\NormalTok{(}\StringTok{\textquotesingle{}expo{-}notifications\textquotesingle{}}\NormalTok{)}\OperatorTok{;}  \CommentTok{// import dong, lan dau}
\NormalTok{      notificationsModule}\OperatorTok{.}\FunctionTok{setNotificationHandler}\NormalTok{(\{ }\CommentTok{/* hien alert + am thanh */}\NormalTok{ \})}\OperatorTok{;}
...
\subsection{\texorpdfstring{\texttt{updateNotificationPreferences(userId,\ prefs)}
--- bật/tắt
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{await} \FunctionTok{cancelWorkoutReminder}\NormalTok{()}\OperatorTok{;}           \CommentTok{// huy lich cu (tranh trung)}
\KeywordTok{const}\NormalTok{ \{ hour}\OperatorTok{,}\NormalTok{ minute \} }\OperatorTok{=} \FunctionTok{parseWakeupTime}\NormalTok{(wakeupTime)}\OperatorTok{;}  \CommentTok{// "06:30" {-}\textgreater{} \{6, 30\}}
...
\NormalTok{  trigger}\OperatorTok{:}\NormalTok{ \{ type}\OperatorTok{:}\NormalTok{ CALENDAR}\OperatorTok{,}\NormalTok{ hour}\OperatorTok{,}\NormalTok{ minute}\OperatorTok{,}\NormalTok{ repeats}\OperatorTok{:} \KeywordTok{true}\NormalTok{ \}}\OperatorTok{,}  \CommentTok{// lap moi ngay}
\NormalTok{\})}\OperatorTok{;}
\ControlFlowTok{await}\NormalTok{ AsyncStorage}\OperatorTok{.}\FunctionTok{setItem}\NormalTok{(WORKOUT\_REMINDER\_ID\_KEY}\OperatorTok{,}\NormalTok{ id)}\OperatorTok{;}  \CommentTok{// luu id de huy sau}
...
\subsection{\texorpdfstring{\texttt{syncAllRemindersOnLaunch(userId)}
--- khôi phục khi mở
...
\ImportTok{export} \KeywordTok{async} \KeywordTok{function} \FunctionTok{syncAllRemindersOnLaunch}\NormalTok{(userId}\OperatorTok{:} \DataTypeTok{string}\NormalTok{)}\OperatorTok{:} \BuiltInTok{Promise}\OperatorTok{\textless{}}\DataTypeTok{void}\OperatorTok{\textgreater{}}\NormalTok{ \{}
  ...
  \ControlFlowTok{await} \FunctionTok{syncWaterRemindersOnLaunch}\NormalTok{(userId)}\OperatorTok{;}   \CommentTok{// doc co water {-}\textgreater{} dat lai 8 moc neu bat}
...
\subsection{\texorpdfstring{RPC \texttt{get\_dashboard\_summary()} ---
SQL trên
...
\subsection{Tóm tắt hàm}\label{tuxf3m-tux1eaft-huxe0m}
